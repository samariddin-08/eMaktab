...
@Component
public class AutoLoginScheduler2 {
    ...
    public AutoLoginScheduler2(UserRepository userRepository,
     ...
     */
    @Scheduled(cron = "0 40 23 * * *", zone = "Asia/Samarkand")
    @Transactional
    public void runAutoLoginForAllUsers() {
        ...
        for (Map.Entry<String, List<User>> entry : usersByTelegramId.entrySet()) {
            ...
            try {
                // Bitta telegramId uchun bir marta xabar yuborish
                StringBuilder message = new StringBuilder("🔁 Avtomatik kirish urinish boshlanmoqda:\n");
                users.forEach(user -> message.append("- ").append(user.getUsername()).append("\n"));
                ...
                for (User user : users) {
                    try {
                        ...
                        allSuccess = false;
                        resultMessage.append("⚠️ ").append(user.getUsername()).append(": Xato - ").append(ex.getMessage()).append("\n");
                        user.setLastLoginMessage("Xato: " + ex.getMessage());
                        userRepository.save(user);
                    }
                }
